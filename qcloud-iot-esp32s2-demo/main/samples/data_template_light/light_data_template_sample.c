...
typedef struct _ProductDataDefine {
    ...
    int saturation;
    int value;
    char m_name[MAX_STR_NAME_LEN + 1];
} ProductDataDefine;
...
static void _init_data_template(void)
{
    ...
    sg_DataTemplate[0].data_property.data = &sg_ProductData.power_switch;
    sg_DataTemplate[0].data_property.type = TYPE_TEMPLATE_BOOL;

    sg_DataTemplate[1].data_property.key  = "hue";
    sg_DataTemplate[1].data_property.data = &sg_ProductData.hue;
    sg_DataTemplate[1].data_property.type = TYPE_TEMPLATE_INT;

    sg_DataTemplate[2].data_property.key  = "saturation";
    sg_DataTemplate[2].data_property.data = &sg_ProductData.saturation;
    sg_DataTemplate[2].data_property.type = TYPE_TEMPLATE_INT;

    sg_DataTemplate[3].data_property.key  = "value";
    sg_DataTemplate[3].data_property.data = &sg_ProductData.value;
    ...
    sg_DataTemplate[4].data_property.data_buff_len = MAX_STR_NAME_LEN;
...
/*control msg from server will trigger this callback kong 控制信息的回调 通过IOT_Template_Register_Property注册*/
...
// register data template properties 注册数据模板属性
...
#ifdef EVENT_POST_ENABLED
    if (eCHANGED == get_property_state(&light->power_switch)) {  //事件
        if (light->power_switch) {
...
static int _get_sys_info(void *handle, char *pJsonDoc, size_t sizeOfBuffer)
{
    ...
    DeviceProperty platform_info[] = {
        {.key = "module_hardinfo", .type = TYPE_TEMPLATE_STRING, .data = "ESP32S2"},
    ...
    DeviceProperty self_info[] = {
        ...
        {.key = "TEST", .type = TYPE_TEMPLATE_STRING, .data = "666"},
        {.key = NULL, .data = NULL}  //end
    };
