...
static  wifi_config_t user_g_wifi_cfg;
...
static int subscribe_topic_wait_result(void *client, DeviceInfo *dev_info, TokenHandleData *app_data)
{
    ...
    //int size = HAL_Snprintf(topic_name, sizeof(topic_name), "%s/%s/data", dev_info->product_id, dev_info->device_name);
    int size = HAL_Snprintf(topic_name, sizeof(topic_name), "$thing/down/service/%s/%s", dev_info->product_id, dev_info->device_name);
...
static int _publish_token_msg(void *client, DeviceInfo *dev_info, char *token_str)
{
    char topic_name[128] = {0};
    //int size = HAL_Snprintf(topic_name, sizeof(topic_name), "%s/%s/data", dev_info->product_id, dev_info->device_name);
    int size = HAL_Snprintf(topic_name, sizeof(topic_name), "$thing/up/service/%s/%s", dev_info->product_id, dev_info->device_name);
    if (size < 0 || size > sizeof(topic_name) - 1) {
        Log_e("topic content length not enough! content size:%d  buf size:%d", size, (int)sizeof(topic_name));
        return QCLOUD_ERR_FAILURE;
    }
...
static void udp_server_task(void *pvParameters)
{
    ...
    while (sg_comm_task_run && --server_count) {
        ...
        if (ret > 0) {
            ...
            int len = recvfrom(server_socket, rx_buffer, sizeof(rx_buffer) - 1, MSG_DONTWAIT, (struct sockaddr *)&source_addr, &addrLen);
...
static void softAP_mqtt_task(void *pvParameters)
{
    ...
    while (sg_mqtt_task_run && (--server_count)) {
        ...
        if (state == EVENT_WIFI_CONNECTED) {
            ...
            if (ret) {
                Log_e("SoftAP: WIFI_MQTT_CONNECT_FAILED");
        ...
        if (!sg_comm_task_run && !sg_token_received) {
            Log_e("something wrong with the comm server task!");
            set_wifi_led_state(WIFI_LED_OFF);
            break;
        }
    }
...
static void smartconfig_mqtt_task(void * parm)
{
    ...
    while (sg_mqtt_task_run && (--wait_count)) {
        ...
        if (state == EVENT_WIFI_CONNECTED) {
            ...
            ret = mqtt_send_token();
            if (ret) {
                Log_e("SmartConfig: WIFI_MQTT_CONNECT_FAILED");
                set_wifi_led_state(WIFI_LED_OFF);
            } else {
...
// #endif
    ...
    /* Get WiFi Station configuration */
     ...
     if(err != ESP_OK)
     {
         Log_i("nvs_open error");
         return ESP_FAIL;
     }
     err = nvs_get_blob(handle, DATA1, &user_g_wifi_cfg, &len); 
     nvs_close(handle);
     Log_i("nvs_open ok");

    if (strlen((const char*) user_g_wifi_cfg.sta.ssid)) {
        *provisioned = true;
        Log_i("Found ssid %s",     (const char*) user_g_wifi_cfg.sta.ssid);
        Log_i("Found password %s", (const char*) user_g_wifi_cfg.sta.password);
    }
    return ESP_OK;
}
...
int start_smartconfig(void)
{
    ...
    app_prov_is_provisioned(&provision);
    if(provision == true)
    {
        ESP_ERROR_CHECK(esp_wifi_set_config(ESP_IF_WIFI_STA, &user_g_wifi_cfg));
         esp_wifi_connect();
         return 0;
    }
...
//send token
    ...
    sg_mqtt_task_run = true;
    ret = xTaskCreate(smartconfig_mqtt_task, SMARTCONFIG_TASK_NAME, SMARTCONFIG_TASK_STACK_BYTES, NULL, SMARTCONFIG_TASK_PRIO, NULL);
    if (ret != pdPASS)  {
